...
2. `games_march2025_full.csv` de Steam (CSV, 94,948 filas, 47 columnas — se usa un subconjunto de 20)
...
|---|---|---|
| `metacritic_score` | "0 si no tiene score" | **95.4% de los registros son 0** (verificado en dos muestras independientes de 500 filas). No es un caso ocasional, es el patrón dominante. |
| `estimated_owners` | rango estimado | **15.8% de los registros tienen el valor literal `"0 - 0"`** — un placeholder sin información real, disfrazado de rango válido |
...
| Nulos detectados por `pandas.isna()` en las 20 columnas núcleo | — | **0% en ambas versiones del archivo** — esto no significa datos limpios, significa que los valores faltantes están representados como placeholders (`0`, `"0 - 0"`, `[]`) en lugar de `NaN` real |
...
**Nota:** el archivo pesa ~450 MB con las 47 columnas originales; se debe leer con `pandas.read_csv(..., usecols=[...])` limitando a las 20 columnas núcleo para que el procesamiento sea manejable.
...
| `Publisher` con valor `"Unknown"` | no mencionado | **Hallazgo nuevo**: existe al menos un caso confirmado (`Dinotopia: The Sunstone Odyssey`) con `Publisher = "Unknown"` como string literal, no como `NaN`. El porcentaje real de "faltantes disfrazados" está subestimado por el 0.35% reportado, ya que ese cálculo solo cuenta `NaN` verdaderos. **Pendiente:** correr `df['Publisher'].value_counts()` sobre el archivo completo para cuantificar cuántos `"Unknown"` existen en total. |
| Duplicados `Name + Platform` | no mencionado | **Hallazgo nuevo**: se confirmó al menos un caso (`Need for Speed: Most Wanted`, dos filas con `Platform = X360` y dos filas con `Platform = PC`). No apareció en una muestra aleatoria de 500, lo que sugiere baja frecuencia, pero no se ha cuantificado el total. **Pendiente:** correr sobre el archivo completo: `df[df.duplicated(subset=['Name','Platform'], keep=False)]` |
...
6. **Aprovechar `developers`/`publishers` completos en las tres consolas** como señal secundaria de desambiguación en el entity-linking por nombre, dado que en vgsales no existe columna de desarrollador.
...
- Atributos exclusivos de Steam (`price`, `metacritic_score`, `positive`/`negative`, `average_playtime_forever`, `peak_ccu`) no tienen clase asignada en la estructura confirmada; si se desean aprovechar, deben añadirse como propiedades de datos directas sobre `Lanzamiento`.
- El género a nivel de `Videojuego` (y no de `Lanzamiento`) es una simplificación: distintas fuentes pueden reportar géneros distintos para el mismo juego. Se requiere una estrategia de reconciliación explícita (unión de valores o prioridad de fuente) que debe documentarse en la evidencia A.
...
| Rango de valores (`sh:minInclusive` / `sh:maxInclusive`) | `Lanzamiento.metacriticScore` entre 1 y 100 (excluyendo 0 como "sin dato") | 95.4% de los registros de Steam son 0, confirmado en dos muestras |
...
- Cuantificar sobre el archivo completo de vgsales: total de duplicados `Name + Platform` y total de `Publisher == "Unknown"`.
- Confirmar si los duplicados de vgsales son idénticos en todos los campos (error de captura) o difieren en algún valor (ediciones/regionales distintas).
