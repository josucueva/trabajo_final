...
STEAM_COLS = [
    ...
    'positive', 'negative', 'average_playtime_forever', 'peak_ccu',
]
...
def load_data():
    with open(os.path.join(BASE_DIR, 'matcheo_data', 'matches.json')) as f:
        matches = json.load(f)
    vgsales = pd.read_csv(os.path.join(DATA_DIR, 'vgsales.csv'))
    steam = pd.read_csv(os.path.join(DATA_DIR, 'games_march2025_full.csv'), usecols=STEAM_COLS)
    console_data = {}
    for src in ['xbox', 'playstation', 'switch']:
        with open(os.path.join(DATA_DIR, f'{src}_games.json')) as f:
...
def add_vgsales_lanzamientos(g, vj_uri, src_data, vgsales_by_name, platform_uris, editor_cache):
    ...
    for orig_name in src_data['vgsales']:
        for row in vgsales_by_name.get(orig_name, []):
            ...
            year = row['Year']
            pub = row['Publisher']
            na, eu, jp, other = row['NA_Sales'], row['EU_Sales'], row['JP_Sales'], row['Other_Sales']
            dedup_key = (plat, int(year) if pd.notna(year) else None, str(pub) if pd.notna(pub) else '',
                         na, eu, jp, other)
...
def add_steam_lanzamientos(g, vj_uri, src_data, steam_by_appid, pc_uri, editor_cache):
    ...
    for appid in src_data['steam']:
        ...
        if pd.notna(row['metacritic_score']) and row['metacritic_score'] > 0:
            g.add((lz_uri, NS.metacriticScore, Literal(int(row['metacritic_score']), datatype=XSD.integer)))
        if pd.notna(row['positive']):
            g.add((lz_uri, NS.reseñasPositivas, Literal(int(row['positive']), datatype=XSD.integer)))
        if pd.notna(row['negative']):
            g.add((lz_uri, NS.reseñasNegativas, Literal(int(row['negative']), datatype=XSD.integer)))
        if pd.notna(row['average_playtime_forever']):
            g.add((lz_uri, NS.tiempoJuego, Literal(int(row['average_playtime_forever']), datatype=XSD.integer)))
        if pd.notna(row['peak_ccu']):
            g.add((lz_uri, NS.peakCcu, Literal(int(row['peak_ccu']), datatype=XSD.integer)))
        if pd.notna(row['release_date']) and row['release_date'].strip():
            lr = BNode()
            g.add((lz_uri, NS.fechaLanzamiento, lr))
            g.add((lr, RDF.type, NS.LanzamientoRegional))
            g.add((lr, NS.region, Literal('WW')))
            g.add((lr, NS.fecha, Literal(row['release_date'].strip(), datatype=XSD.date)))


def add_console_lanzamientos(g, vj_uri, src_data, console_by_id, console_src, plat_uri, editor_cache):
...
def process_doom(g, norm_name, src_data, vgsales_by_name, steam_by_appid, console_by_id,
    ...
    if norm_name == 'doom':
        ...
        add_vgsales_lanzamientos(g, vj_orig, src_data, vgsales_by_name, platform_uris, editor_cache)
        steam_only = {'steam': src_data.get('steam')}
        add_steam_lanzamientos(g, vj_2016, steam_only, steam_by_appid, pc_uri, editor_cache)
        for src in ['xbox', 'playstation', 'switch']:
            if src_data.get(src):
                sub = {src: src_data[src]}
                add_console_lanzamientos(g, vj_2016, sub, console_by_id, src, platform_uris[f'_{src}'], editor_cache)
    elif norm_name == 'doom 2016':
        vj_2016 = NS.V_doom_2016
        if (vj_2016, None, None) not in set(g.triples((vj_2016, RDF.type, None))):
            g.add((vj_2016, RDF.type, NS.Videojuego))
            g.add((vj_2016, NS.nombre, Literal('DOOM (2016)')))
        add_vgsales_lanzamientos(g, vj_2016, src_data, vgsales_by_name, platform_uris, editor_cache)
        for orig_name in (src_data.get('vgsales') or []):
            for row in vgsales_by_name.get(orig_name, []):
                if pd.notna(row['Genre']) and row['Genre']:
                    g_uri = get_or_create(g, genero_cache, 'G', NS.Genero, row['Genre'].strip())
                    g.add((vj_2016, NS.perteneceAGenero, g_uri))
...
def main():
    ...
    steam_by_appid, vgsales_by_name, console_by_id = build_indices(vgsales, steam, console_data)
    ...
    g.bind('owl', OWL)
    g.bind('xsd', XSD)
    g.parse(os.path.join(BASE_DIR, 'ontologia.ttl'), format='turtle')
